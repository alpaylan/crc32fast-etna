...
(cd ${workload_path} &&     CARGO_INCREMENTAL="0" RUSTFLAGS="-C instrument-coverage -C link-dead-code -C codegen-units=1 -C inline-threshold=0 -C llvm-args=-inline-threshold=0 -C debuginfo=2" cargo build --release --bin etna-faultloc --features etna)
...
(cd ${workload_path} &&     crabcheck-profiling-fast-analyze coverage crc32fast ./target/release/etna-faultloc)
